class DeadlockDetection:
    ...
    def input(self):
        self.processes = int(input("Enter no. of processes: "))
        self.resources = int(input("Enter no. of resources: "))
    ...
    def show(self):
        ...
        for i in range(self.processes):
            ...
            print("       ")
            for j in range(self.resources):
                print("  " + str(self.allocate[i][j]))
            print("       ")
            for j in range(self.resources):
                print("  " + str(self.max[i][j]))
            print("       ")
            if i == 0:
                for j in range(self.resources):
                    print("  " + str(self.available[j]))
